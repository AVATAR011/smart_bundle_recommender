...
function computeBundleRisk(addons: any[], ctx: any) {
  if (!addons.length) return 15; // base TP-only risk floor

  const addonRisk =
    addons.reduce((sum, a) => sum + (a.riskScore ?? 0), 0) / addons.length;

  const telematicsPenalty =
    (ctx.telematics?.overspeed_score ?? 0) > 70 ? 10 : 0;

  const claimPenalty = (ctx.customer_risk?.claim_count_3y ?? 0) > 1 ? 15 : 0;

  return Math.min(
    100,
    Math.round(addonRisk + telematicsPenalty + claimPenalty),
  );
}
...
export const recommendRoutes: FastifyPluginAsync = async (app) => {
  app.post("/recommend-bundles", async (req, reply) => {
    try {
      const requestId = (req.body as any)?.request_id || crypto.randomUUID();
      ...
      if (cached) {
        app.log.info(
          { request_id: requestId, cached: true },
      ...
      const candidates = templates.map((tpl) => {
        ...
        const want =
          tpl.key === "protection_plus" ? 5 : tpl.key === "value" ? 4 : 2;
        ...
        for (const s of sorted) {
          if (chosen.find((c) => c.name === s.name)) continue;
          if (chosen.length >= want) break;
          chosen.push(s);
        }

        const priceScoreSum = chosen.reduce(
          (n, x) => n + (x.priceScore ?? 0),
          0,
        );
        ...
        return {
          ...
          premium_estimate_band: bandFromPrice(
            priceScoreSum,
            ctx.constraints?.budget_band,
          ),
      ...
      app.log.info(
        {
          ...
          llm_ms: latencyMs,
          usage,
          bundle_ids: Object.values(validated.bundles).map(
            (b) => b.bundle_name,
          ),
        },
        "recommendation done",
